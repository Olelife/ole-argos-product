...
const items = dirs.map(slug => {
  ...
  return {
    ...
    dudasOpen: dudas.filter(dudaOpen).length, dudasTot: dudas.length,
    storiesClosed: stories.filter(r => storyClosed(r, fm.jira_goal_status)).length, storiesTot: stories.length,
  };
});
...
P.items.forEach(function(d){
  ...
  el.innerHTML='<div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px;">'
    +'<span style="font-weight:500; font-size:15px;">'+T(d.title)+'</span>'
    +'<span style="font-size:11px; font-weight:500; padding:2px 9px; border-radius:999px; white-space:nowrap; background:'+c[0]+'; color:'+c[1]+';">'+T(d.status)+'</span></div>'
    +'<div style="display:flex; flex-wrap:wrap; gap:6px; font-size:12px; color:var(--text-secondary);">'
    +'<span style="background:var(--surface-1); border-radius:6px; padding:2px 8px;"><i class="ti ti-help-circle" aria-hidden="true"></i> '+d.dudasOpen+'/'+d.dudasTot+' dudas abiertas</span>'
    +'<span style="background:var(--surface-1); border-radius:6px; padding:2px 8px;"><i class="ti ti-list-check" aria-hidden="true"></i> '+d.storiesClosed+'/'+d.storiesTot+' historias cerradas</span>'
    +'<span style="background:var(--surface-1); border-radius:6px; padding:2px 8px;">PRD '+T(d.prd)+' · Figma '+T(d.figma)+'</span></div>'
    +'<div style="display:flex; gap:8px; margin-top:2px;">'
    +'<button class="op" data-s="'+T(d.slug)+'" style="display:flex; align-items:center; gap:6px;"><i class="ti ti-folder-open" aria-hidden="true"></i> Abrir ↗</button>'
    +'<button class="tk" data-s="'+T(d.slug)+'" style="display:flex; align-items:center; gap:6px;"><i class="ti ti-brand-jira" aria-hidden="true"></i> Tickets ↗</button></div>'
    +'<div style="font-size:11px; color:var(--text-muted);">actualizado '+T(d.updated)+'</div>';
  grid.appendChild(el);
});
